...
function App() {
  ...
  const fuse = useMemo(() => {
    return new Fuse(entries, {
      keys: ['title', 'description', 'companies', 'tags'],
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // ESC to clear search or go home
      if (e.key === 'Escape') {
          if (isCmdOpen) {
              setIsCmdOpen(false);
  ...
  const counts = useMemo(() => {
    ...
    Object.values(Category).forEach(cat => tempCounts[cat] = 0);
    entries.forEach(e => {
          ...
          <>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 animate-in fade-in duration-300 gap-4">
                <div className="flex items-baseline gap-3">
                    ...
                    <span className="hidden sm:inline text-xs font-mono text-zinc-500 bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-800">
                        REQ_ID: {reqId}
                    </span>
                </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-lg bg-zinc-50 dark:bg-zinc-900/10">
                    <p className="text-zinc-500 text-lg font-light">No failures found matching parameters.</p>
                    <button 
            ...
            <div className="relative group cursor-text" onClick={() => setIsCmdOpen(true)}>
              ...
              <div className="relative flex items-center">
                ...
                <div className="absolute inset-y-0 right-0 pr-2 flex items-center pointer-events-none">
                    <kbd className="inline-flex items-center border border-zinc-300 dark:border-zinc-700 rounded px-2 text-[10px] font-sans font-medium text-zinc-500 bg-zinc-200/50 dark:bg-zinc-800/50">
                    ⌘K
                    </kbd>
                </div>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 flex flex-col relative z-10">
        <div className="flex flex-col md:flex-row gap-8 flex-1">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {/* Global Footer attached to main content area */}
            <Footer />
          </div>
        </div>
      </main>

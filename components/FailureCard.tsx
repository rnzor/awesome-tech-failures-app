...
const getBorderColor = (category: Category) => {
  switch (category) {
    case 'security': return 'rgb(239, 68, 68)'; // red-500
    case 'outage': return 'rgb(245, 158, 11)'; // amber-500
    case 'ai-slop': return 'rgb(168, 85, 247)'; // purple-500
    case 'startup': return 'rgb(113, 113, 122)'; // zinc-500
    case 'decision': return 'rgb(234, 179, 8)'; // yellow-500
    case 'product': return 'rgb(59, 130, 246)'; // blue-500
...
*Generated via Awesome Tech Failures*`;
  ...
  return (
    <div
      ...
      className="group relative rounded-xl bg-zinc-50 dark:bg-black border border-zinc-200 dark:border-zinc-800 p-6 cursor-pointer overflow-hidden transition-colors h-full flex flex-col"
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {/* Root Cause Section */}
        {entry.root_cause && (
          <div className="mb-4 bg-red-50 dark:bg-red-900/10 p-3 rounded-lg border border-red-100 dark:border-red-900/20">
            <span className="text-[10px] uppercase tracking-wider text-red-600 dark:text-red-400 font-bold block mb-1">Root Cause</span>
            <p className="text-zinc-800 dark:text-zinc-200 text-sm font-medium line-clamp-2">{entry.root_cause}</p>
          </div>
        )}
